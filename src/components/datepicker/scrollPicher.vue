    ...
    >
      <div
        ...
        :style="{
          ...
          transform: 'rotateX(' + realRotate + 'deg)'
        }"
      >
        <div
...
export default {
  data: function() {
    return {
      stageHeigth: 320, //滚动高度
      itemHeight: 50, //每个div高度
      rotateRate: 0, //每次旋转角度
      itemNum: 12, //单圈最大容纳数目
      zDistance: 0, //轴距
      realRotate: 0, //实际旋转度数
    ...
    handleEvent: function(e) {
      ...
      if (e.type === "touchmove") {
        ...
        if (this.maps.length < this.itemNum) {
          ...
          if (
            this.realRotate <
            -this.rotateRate * (this.maps.length - 1) - this.rotateRate / 2
          ) {
            this.realRotate =
              -this.rotateRate * (this.maps.length - 1) - this.rotateRate / 2;
          }
